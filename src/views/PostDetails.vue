...
export default defineComponent({
  setup() {
    ...
    // Sort comments by the date they were created
  ...
  // Ensure both user.value and reply.user are defined before accessing their IDs
  ...
  try {
    ...
    // Find the parent comment and push the new reply into its children array
    const addReplyToComment = (comment: Comment) => {
      if (comment.id === parentCommentId) {
        ...
        comment.showReplies = true;  // Show replies after submission
        return true;
      }
      if (comment.children) {
